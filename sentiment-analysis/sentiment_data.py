...
class SentimentExampleBatchIterator:
    ...
    def get_next_batch(self):
        if self._cur_idx < len(self.data): # loop over the dataset
            ...
            maxLength = max(len(x.words) for x in batch_exs )
            for x in batch_exs:
              if(len(x.words) < maxLength):
                a = x.word_indices + list(np.repeat([0], (maxLength - len(x.words))))
              else:
                a = x.word_indices
              i.append(a)
              l.append(len(x.words))
              lb.append(x.label)
              
            batch_inputs = np.array(i)
            batch_lengths = np.array(l)
            batch_labels = np.array(lb)
